resource "azurerm_monitor_scheduled_query_rules_alert" "anmla" {
    for_each                = var.aks_node_memory_limit_alerts
    name                    = "Node Avg Memory Threshold Exceeded %{ if each.value["clusterName"] != "*" } for Cluster ${each.value["clusterName"]} %{ endif }"
    description             = "Alert for when the Memory theshold for Nodes associated with the cluster goes above the defined limit."
...
output "aks_node_memory_limit_alerts_ids" {
    value = [for x in azurerm_monitor_scheduled_query_rules_alert.anmla : x.id]
}