locals {
  ...
  vm_state_exists            = length(values(data.terraform_remote_state.virtualmachine.outputs)) == 0 ? false : true
}
...
locals {
  ...
  linux_vm_custom_script_list = flatten([
  ...
  linux_vm_custom_scripts = {
    for script in local.linux_vm_custom_script_list :
...
resource "azurerm_virtual_machine_extension" "custom_script" {
  ...
  virtual_machine_id         = local.vm_state_exists == true ? lookup(data.terraform_remote_state.virtualmachine.outputs.linux_vm_id_map, each.value.virtual_machine_name) : lookup(data.azurerm_virtual_machine.this, each.value.virtual_machine_key)["id"]
  ...
  tags = local.vm_state_exists == true ? lookup(data.terraform_remote_state.virtualmachine.outputs.linux_vm_tags_map, each.value.virtual_machine_name) : (local.resourcegroup_state_exists == true ? lookup(data.terraform_remote_state.resourcegroup.outputs.resource_group_tags_map, var.resource_group_name) : data.azurerm_resource_group.this.0.tags)
...
resource "azurerm_virtual_machine_extension" "storage" {
  ...
  virtual_machine_id         = local.vm_state_exists == true ? lookup(data.terraform_remote_state.virtualmachine.outputs.linux_vm_id_map, each.value.virtual_machine_name) : lookup(data.azurerm_virtual_machine.this, each.key)["id"]
...
resource "azurerm_virtual_machine_extension" "network_watcher" {
  for_each                   = var.enable_network_watcher_extension == true ? var.virtual_machine_extensions : {}
