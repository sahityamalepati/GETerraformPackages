...
echo 'description = "Map of trafficmanager_enable resource ids"' >> resource_ids.tf
echo 'default = {' >> resource_ids.tf
az resource list --tag trafficmanager_enable=true --query []."[name, id]" --output json | sed 's/\[//g; s/\],/\n/g; s/",/" =/g; s/\]//g'| awk NF=NF RS= >> resource_ids.tf
