...
data "azurerm_application_security_group" "linux_nics_with_asg" {
...
resource "azurerm_network_interface_application_security_group_association" "this" {
  for_each                      = local.linux_nics_with_asg
  network_interface_id          = lookup(azurerm_network_interface.linux_nics, each.value["nic_key"]).id
  application_security_group_id = data.azurerm_application_security_group.linux_nics_with_asg[each.key].id
